 ...
 * ADR-0003 decision 13 — keyed on the range's `releasedUpTo` bound being
 * `null`, reviewer m-4, never on the attribution's released-identity count),
 * then the intent routing (D17(4)(5)): exact-duplicate intents collapse
 * first; `promote` × `release-anyway` on one line is an operator
 * contradiction; a `promote` over an in-flight prerelease is the P-03
 * release with an inherited (empty) change set and `bump: null`, and over
 * release-worthy pending changes or a pointer not held by a prerelease it
 * refuses; a `release-anyway` over a quiet line is the `forced` record —
 * never a routine release; a `release-as` demand the kernel grammar cannot
 * parse refuses regardless of the runway. Thereafter: `refused` on the
 * remaining operator contradiction (§2.9, S-01: an explicit `release-as`
 * demand against a runway with nothing release-worthy), the recorded no-op
 * with the ignored commits enumerated in input order (§2.9, PL-06), or the
 * `release` record whose kernel change set is constructed through
 * `ChangeSet.of` — a kernel construction rejection surfaces as a `refused`
 * record and is never re-thrown (§2.9, ADR-0003 decision 1).
 *
...
export const decideLine: DecideLine = (line, input, range) => {
  ...
  // Precedence 2 — promote × release-anyway (§2.9): one line cannot both
  ...
  // Precedence 3 — the promotion (D17(4), P-03): over an in-flight
  ...
  // Precedence 4 — the forced record (D17(5), S-01): a release-anyway over a
  ...
  // Precedence 5 — a release-as demand the kernel grammar refuses is an
  ...
  // Precedence 6 — operator contradiction (§2.9, S-01): an explicit
  ...
  // Precedence 7 — the recorded no-op (§2.9, PL-06, S-01): with no
  ...
  // Precedence 8 — the release (§2.7, §2.9): the contributing commits are
