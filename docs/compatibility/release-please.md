...
| `always-bump-patch` / `always-bump-minor` / `always-bump-major`                              | Dedicated strategies                                 | `release-anyway` intent + declared mapping cover the outcome on a release-worthy range only — a quiet line assembles no release (decision D66, [#271]); strategy kinds are not ported | `PARTIAL`        | Intent + mapping                                                   | Decision D2: map onto policy, never port the strategy taxonomy; the quiet-line outcome is uncovered, held open by decision D66 ([#271]) | [decide.test.ts], [#271]                                                                             | `shadow` |
