...
1. **Plan** — the planner's pure door over the caller's closed input. A
   ...
   phase 2 §2.9's negative outcomes are records). So is a planning
   decision of kind `blocked` for the executed line (#263): the run
   returns the `blocked(cause)` row with the plan named, `handle` null,
   and `drives` empty — the attempt never opens, the claim never
   acquires, nothing mints. The cause carries the decision's cause token
   and its detail sentence (the replay posture: a released version is
   never re-planned).
