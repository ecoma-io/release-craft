...
| `command` (a door selector)                                         | `run` only — `plan`-only is the CLI's own door, reachable by any workflow that runs the bin directly. The cross-process posture split with issue #194 (phase 12 §2.7): a fresh process's `resume` of a dead holder's attempt now works across invocations, but `resolve`, `abort`, and `show attempt` still return `refused(unknown attempt)`, and `run` remains the only input that avoids a guaranteed refusal — the inventory is revisited when the surviving doors' reconstruction lands (§7) |
...
- **The durable attempt lookup** (phase 11 §4, question 6) has landed for
  `resume` (issue #194) — a fresh process's cross-process resume works;
  the remaining carried-attempt doors still refuse from a fresh process.
  The `command` input's refusal ([§2.3](#23-the-inputs-action-metadata-onto-the-closed-grammar))
  remains the v1 posture — the inventory may grow with the doors it can
  honestly serve, in its own reviewed slice.
