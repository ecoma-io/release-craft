...
Phase 10 adds no engine behavior. Every mechanism the matrix exercises
already landed: the deterministic planner (ADR-0003/0004), the execution
...
VerticalMatrix {
  ...
  artifacts: readonly ArtifactStep[5]  // §3.4 — one per class, phase 7 §2.1 shape
...
existing tags), and its line policy. A `ChannelFixture` names a channel
...
`stable`, `beta`, `rc`, `next`, `lts`. The matrix asserts each one's
target after every checkpoint run: `stable` ends at `5.0.0` (moving from
`4.9.2` through the ladder's promote), `beta` at `5.0.0-beta.2`, `rc` at
`5.0.0-rc.1`, `next` ahead on its own stream, `lts` at the `1.9-lts`
cut. A channel whose target a run should not move asserts unchanged —
a promotion on `main` never drags `lts` with it.
...
1. `5.0.0-beta.1` — the beta stream opens (P-02's fresh sequence per
   identifier).
2. `5.0.0-beta.2` — only the beta stream advances (P-06); an `rc` run
   demanded between the two still lands `rc.0` from its own key.
3. `5.0.0-rc.1` — the rc stream; a concurrent beta denial carries the
   winner's sequence as its retry base (E-08, ADR-0011 decision 7).
4. `5.0.0` — the promote door: empty change set, `bump: null`, the
   promoted-from record, the rc stream closes (P-03).

Beside the ladder, `4.8.x` cuts `4.8.7` from its own tags (P-07's
maintenance shape), and `3.x` / `2.x` release their own minors/patches
independently — no cross-dependency between any two lines' runs (M-02).
A `stable-version` claim on any line is a record, not a lease: its
release is a no-op and a later verify still reads held (P-01).
...
Five artifact steps, declared in the phase 7 §2.1 shape — `kind` and
`coordinates` opaque, recorded verbatim, never dereferenced:
...
| `publish-all` | `publication` | `container`, `sbom` | evidence-present            |
...
validation, and the generations record the walk. The five classes are
the matrix's declared labels, nothing more — the engine sees opaque
strings.
...
- **Retried** — `hook:sign`: fails once, then a resume continues at the
  hook step (the failed step is uncompleted; `classifyResume` walks to
  it) and the second observation lands. A completed hook is never
  re-executed — the ledger projection answers the replay.
...
`verify` — plus both hook windows of §3.5, plus one window inside the
artifact walk (between `sbom` and `publish-all`). Every window is
`crash` unless §3.5 names it `failure`. The full-matrix run then
completes: every window classified on the next run, every resume
landing the same tail as the uninterrupted run's (§6 recovery).
...
gets at least its in-memory proof here.

### 10.3 — the ledger-backed replay vertical

The same matrix through the ledger's doors: the record-path replay door
(Phase 5 §2.8) reconstructs state from the recorded tail between runs —
never from in-process memory. Interruptions classify through
`classifyResume` and the crash classes (E-01); the replayed tail is
byte-identical to the uninterrupted run's; a completed step replays
...
The assembled adapter (ADR-0010's `openGitHubAdapter`, injected
transport) over the matrix: publication derives every remote row from
the recorded evidence alone (the changelog seam), the tag mint pushes
through the binding's CAS, and reconciliation compares the remote
against the binding per R-10..R-13 with D30's per-listing observation
outcomes — a listing failure demotes nothing, and an out-of-band remote
mutation is detected on the next reconcile.
...
- **Write-ahead** (ADR-0006 decision 2): every step's start is durable
  ...
  there.
- **No wall clock** (E-05/E-10; ADR-0009 decision 4): no fixture, no
...
| --- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| V1  | Plan integrity      | Same inputs plan identically; the executed steps are exactly the planned canonical sequence; the fingerprint verifies (E-03, invariant 12).                                      | 10.2–10.5  |
...
| V3  | Prerelease sequence | The ladder advances per stream §3.2; only the demanded stream moves; a denial carries the winner's sequence as the retry base (E-08).                                            | 10.2–10.5  |
| V4  | Promotion           | The promote run lands `5.0.0` with the promoted-from record, closed rc stream, and moved channel targets — and moves no other line's channel (P-03).                             | 10.2–10.5  |
...
| V7  | Recovery            | Every §3.6 window classifies and resumes from the recorded tail — no completed step re-executes, no uncompleted step skips, the resumed tail equals the uninterrupted tail.      | 10.2–10.5  |
...
| V10 | Reconciliation      | The remote surface derives from recorded evidence alone and reconciles per R-10..R-13 with D30's outcomes; an out-of-band mutation is detected.                                  | 10.5       |
...
- The concurrency rows (V8) use the ADR-0011 harness pattern — the
  hostile runner, the arm-gated interception, the fired-flag worlds —
  lifted to the vertical's claim path; a serial-only concurrency proof
  is the #47 class and refuses review.
