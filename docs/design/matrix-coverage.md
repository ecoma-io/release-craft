...
| E-07                                                                       | execution-only | owned (phase 8)             | Phase 4 (full, domain level): claim scopes, claim–verify–write, deterministic winner/loser, `abandoned(follower-of:…)` (ADR-0005 decisions 4–5; phase4 contract §2.3–§2.4) — the physical tag CAS is the phase 8 contract (ADR-0009 §2.3, D24): git's ref creation as the claim store's atomic accept; fixtures pin with the implementation PR                                                             |
