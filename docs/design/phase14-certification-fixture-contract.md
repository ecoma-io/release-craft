...
| I6     | resume after abort         | ADR-0013 decision 3 — terminal from the ledger alone, and a later resume of the carried attempt throws the named violation; from a fresh process, `resume`'s durable reconstruction (issue #194) meets the tail's recorded abandonment — the same thrown violation — while `resolve`/`abort` refusing `unknown attempt` first remains the attempt store's posture for the doors without the reconstruction (phase 11 §2.7, phase 12 §2.7) |
...
Thirty-four cells earn pins. The manifest
...
| `x-01`      | —                                                     | A-cross-process       | CLI                                        | exit 10, pass-through equality with a fresh engine's value                                               | The carried-attempt doors from a fresh process: `resume` takes over a dead holder's attempt (issue #194's durable reconstruction — published, re-minting the recorded tag), while `resolve`, `abort`, and `show attempt` render `refused(unknown attempt)` — pinned as pass-through, not as a verdict (phase 12 §6.7's pin, cell-ized): the assertion is equality with what a fresh engine returns, so the durable attempt lookup moves the value and not the cell ([§5](#5-the-change-protocol)).                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- `resolve`, `abort`, and `show attempt` refuse — the engine carries no
  attempt for the plan; the rendered value is the boundary's own
  `refused(unknown attempt)` at exit 10, quoting the handle (verified:
  `carriedEntry` in `src/app/engine.ts`). `resume` no longer refuses:
  its durable reconstruction (issue #194) re-assembles the plan from the
  request's own closed input, re-claims the dead holder's attempt
  idempotently, and republishes the recorded tag. All four pinned as
  `x-01`.
...
So the cross-process leg is not uniformly refused — and since issue #194,
not uniformly answered per cell either: `x-01` pins a split answer (one
takeover, three refusals), `x-02` and `x-04` work, `x-03` refuses. The
fixture pins all four rather than the blanket "today, that
is `refused(unknown attempt)`" — the blanket sentence (phase 12 §2.7) was
true of the carried-attempt doors and false of the observation and the
fresh-run scan, and a fixture that pinned the blanket would have gone stale
the day the durable lookup landed without any shape having moved
dishonestly.

**The declared change, landed for `resume`.** The durable attempt lookup
(phase 11 §4 question 6) landed for `resume` in issue #194: one of `x-01`'s
four carried-attempt renderings now works across invocations with zero
grammar change — with `--line` named, since the reconstructed attempt's
`planLine` comes from the request (the carried entry that was authoritative
for the line is gone). The fixture moved with the value: `x-01`'s resume
leg asserts published, and the pass-through equality (what a fresh engine
returns for the same handle) now compares two published values. The other
three renderings — `resolve`, `abort`, and `show attempt` — keep their
`refused(unknown attempt)` value; when their reconstruction lands, the
equality holds with a different value and the fixture does not move again —
the slice that lands it regenerates the affected expectations and flips the
manifest rows' notes as part of its own obligations. The Action's `command`
input refusal (phase 13 §2.3, §7) is tied to the surviving posture and
moves when those doors move.
...
2. **Matrix breadth versus suite runtime.** The repository's suite already
   ...
   **40 live cells** — the §3.3 table's thirty-four leaves headroom for the
