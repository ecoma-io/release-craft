...
- **No direct side — assert bytes only.** Refused: pass-through equality is
  ...
  dropping it would let a translation, default, or gate the CLI adds break
  the equality loudly (phase 12 §6.3) with no fixture there to hear it.
...
| I6     | resume after abort         | ADR-0013 decision 3 — terminal from the ledger alone, and a later resume of the carried attempt throws the named violation; the fresh process refusing `unknown attempt` first is the attempt store's posture (phase 11 §2.7, phase 12 §2.7) |
...
| I11    | the declared lie           | phase 12 §2.5 — an unobserved feedRef faults at the planner's range classification (exit 70, nothing executed); phase 12 §2.4 — an unobserved ref head faults at the mint (exit 70, records standing)                                        |
...
A cell is a (walk, window, assembly) triple — or one of two **declared
posture rows**, which the table earns beside the triples:

- a **windowless posture row** pins a property of the assembly or the
  surface itself rather than a window's outcome: the determinism sweep, the
  store-less corner, the exit table's census row, the cross-process
  postures, the no-verdict row. Windowless by design — R0 does not reach
  these rows, because R0 governs window claims and a posture row makes
  none; it appears in the manifest with its posture named;
- a row whose **carrier is the repository** rather than one of §3.1's three
  assemblies — `action-06`, the drift row, pairs the composite's declared
  toolchain values with the repository's pin files. Not a fourth assembly:
  the carrier is the same repository phase 13 §6 fixture 1's drift row
  reads.

A triple earns a pinned fixture only under all of:
...
- **R2 — the assembly earns it by a distinguishing property, or by being
  the only assembly the cell can be produced on without a port bypass.** A
  cell pinned on `A-memory` must exercise memory's determinism or its
  no-mint or store-less posture — or seat a fault at the port seam, which
  only the memory factory admits: `assembleMemoryStores` takes its stores
  as declared parameters, so the registered wrappers and the declared
  injections seat there, where the git binding builds its stores inside its
  own factory and a wrapper beneath them would be a port bypass
  ([§8](#8-what-the-fixture-never-does)). A cell pinned on `A-git` must
  exercise durability — real recorded refs, a real crash's restart, a
  filesystem seed — or need the mint, which memory does not wire (I12's
  minting line has no memory home). On `A-cross-process`, the process-local
  boundary. A cell that runs identically on two assemblies with neither a
  distinguishing property nor such an only-home is pinned once, on the
  cheaper one.
...
Thirty-six cells earn pins. The manifest
...
contract sections, and its status (`live`, `typed-row`, `census-only`, or
`refused`). Cell ids are
...
| `memory-10` | all memory cells                                      | A-memory              | CLI + boundary                             | byte identity                                                                                            | The zero-random assembly's determinism: every memory envelope is byte-identical across runs with no projection — the determinism half of the pairing `git-15` asserts (the proof that the projection rule ([§4.3](#43-the-generation-mechanism)) is the only delta between recorded and live git bytes lives in `git-15`'s cell, executable, not cited from this row).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `git-11`    | I6                                                    | A-git                 | boundary (thrown) + CLI (refused 10)       | the two renderings of one doctrine                                                                       | ADR-0013 decision 3 — terminal from the ledger alone, the named violation on a later resume of the carried attempt — with the attempt store's posture beside it (phase 11 §2.7, phase 12 §2.7): where the engine still carries the attempt, the later resume throws the named violation; from a fresh process the carried-attempt door refuses `unknown attempt` first. One doctrine, both renderings pinned, neither translated into the other.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `git-12`    | W1 × I11                                              | A-git                 | CLI                                        | exit 70 twice, distinct rows, stdout empty                                                               | The declared lie's two fault bands, each to its recorded section: the unobserved feedRef faults at the planner's range classification before anything executes (phase 12 §2.5); the unobserved ref head faults at the mint after the walk's records stand (phase 12 §2.4). Two fault-band rows that must never collapse into one cell — the second leaves recorded evidence, the first leaves none.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `git-13`    | —                                                     | A-git (nominally)     | CLI                                        | typed rows + renderings                                                                                  | Census-only, declared: the exit table's seven unreachable-today rows (`satisfied-externally`, `resolved`, `abandoned`, `blocked`, `failed`, `stale`, `escalate` — exactly phase 12 §6.2's class) are unreachable through the one-shot, declarations-less process transports — phase 12 §6.2's exact letter; the boundary transport renders no exits at all, which is why `memory-07`/`memory-08` are not this row's contradiction — and the import law of [§8](#8-what-the-fixture-never-does) bars the fixture from `src/cli/`, where a renderer-level pin would live, so this row produces no test. Its enforcement is named in its manifest row: the exit-table cross-pin in the CLI's own suite plus this contract's recorded copy of the table ([§4.2](#42-the-classes)). The row exists so the fixture's census of the table is total, and it is exempt from the produced-by-a-test rule ([§5](#5-the-change-protocol)). |
...
| `git-15`    | —                                                     | A-git + A-memory      | boundary                                   | ledger projections                                                                                       | The projection-is-the-only-delta claim ([§4.3](#43-the-generation-mechanism)) made executable instead of cited: one declared world driven through BOTH assemblies — the memory engine and the git binding — with the identity chain's sha256 values committed as literals (`inputs_sha256:…`, `plan_sha256:…`, `attempt_sha256:…`), the engine's attempt id asserted equal to the repository's own ledger ref, and the git ledger's tail — read with the fixture's raw `git` spellings, never the product reader — equal to the memory ledger's after each side's claim token is projected (#186).                                                                                                                                                                                                                                                                                                                             |
...
| `x-05`      | W5 × I1, two live processes                           | A-cross-process       | CLI                                        | exit 11 naming the taker, the supersession record durable                                                | The takeover fence across two live processes (#227; phase 4 §2.4 item 6): the second `run` acquires past the standing lease, the register records the supersession naming both holders, and the suspended holder's resumed walk answers `denied` naming the taker — the durable record read fixture-side with the raw `git` spellings, never the product reader.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `action-05` | W2                                                    | A-git                 | Action                                     | envelope ≡ the clean run, byte for byte                                                                  | Phase 13 fixture 4's scenario cell: the planted ambient is fixture 4's, verbatim — lying `GITHUB_*` values, `ACTIONS_*`, `RUNNER_*`, `CI=true`, `INPUT_WORLD` naming a different document, `GIT_DIR` pointing elsewhere, a `NODE_OPTIONS` carrying a marker, a token-shaped `GH_TOKEN` — and no planted value is reachable in the envelope, the annotation, or the conclusion.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Any hook or artifact window (I3ext) on a process transport            | The CLI v1 executes the empty declaration unless the opt-in `--changelog` declares the single changelog artifact (#327); no built-bin test drives an extension-step crash through it, so the window stays unproven at the process transports. The Action inherits the empty declaration (phase 13 §2.3's refused `declarations`). The window is the boundary transport's, with declared injection. R1. |
...
- Two live `run` processes racing one prerelease stream do not inherit each
  other's authority silently — an acquisition past a standing lease lands a
  supersession record in the register naming both holders (#227), and the
  passed holder's next verification answers `denied` naming the taker at
  exit 11. Pinned as `x-05`.

So the cross-process leg is not uniformly refused: two refusal cells, two
working cells, and the takeover fence that lets one working run pass
another's lease only on the record. The fixture pins all five rather than
the blanket "today, that
is `refused(unknown attempt)`" — the blanket sentence (phase 12 §2.7) is
true of the carried-attempt doors and false of the observation, the
fresh-run scan, and the fence, and a fixture that pinned the blanket would
go stale on the day the durable lookup landed without any shape having
moved dishonestly.
...
and its predicate is pinned exactly:

- the repository path is rewritten to `REPO`, in plain and JSON-escaped
  text spellings (the projection runs over envelope text, which the
  expected file's own JSON escapes once more);
- a JSON string whose parsed value is **exactly** 64 lowercase hex
  characters — the claim token's whole shape — is rewritten to `CLAIM`.

The predicate is whole-value, never substring, and that is load-bearing:
...
the one value in the system whose entire content is the random hex, so the
rule has exactly one target. The rule is applied identically at generation
and at comparison, so the comparator holds no cleverness and the committed
bytes are exactly what a reviewer reads. The memory assembly needs no
projection (its claim tokens are a seeded counter — `claim:1`, verified:
`MemoryClaimStore`). That is what makes the pairing executable — cell
`git-15` drives ONE declared world through both assemblies and asserts the
git ledger's projected records equal to the memory ledger's, with the
identity chain's sha256 values committed as literals — the proof that the
projection is the only delta, run rather than cited (`memory-10` and
`git-01` supply the determinism halves it builds on): if a second random
value ever entered the git path, the pairing would flake loudly rather than
pin a lie.
...
ManifestRow {
  ...
  kind                // "windowed" | "posture" — §3.2's two row shapes
  ...
  status              // "live" | "typed-row" | "census-only" | "refused"
  reachability        // for typed rows: the declared-unreachable note and its mover
  enforcement         // for census-only rows: where the pin lives instead
  refusal             // for refused rows: the refusing rule and its carrier scope
}
```
...
- a `live` or `typed-row` row's cell id is not produced by any test (a
  census row without a pin). The two exempt statuses carry their own named
  enforcement instead: a `census-only` row (`git-13`) names where its pin
  lives — the exit-table cross-pin in the CLI's own suite plus the
  fixture's recorded copy of the table — and a `refused` row names the
  rule that refuses it;
- the **executable census** fails — R0 made law over the manifest, not
  over this document's prose. Every row's window claim is a
  (window, carrier-scope) pair: an earned row claims its window at its
  transports; a refused row claims its window at the scope its refusal
  names (so a window refused on the process transports and earned on the
  boundary is two pairs, not a contradiction — I3ext's exact shape). The
  check: `earned ∪ refused == taxonomy` — every recorded window of
  [§3.1](#31-the-axes) claimed by at least one pair — and
  `earned ∩ refused == ∅` — no (window, scope) claimed by both, where
  "earned" means every non-refused status (`live`, `typed-row`,
  `census-only`). A window
  quietly omitted from both is a red suite, not a recorded-evidence gap
  shipping as certified; a window both earned and refused at the same
  scope is a contradiction the suite refuses to compile.
...
3's byte-equality and parse replay, fixture 4's hostile leg — and this
contract owns the scenario list and expected data for that subset, the four
fixtures whose rows are instantiated data. Fixture 8's metadata gate has no
scenario data to hand over — its subject is the composite's own declared
values checked against the repository's pin files — and stays with the
Action implementation (phase 13 §6, §7):
...
- The **scenario set** is this contract's git-assembly cells driven through
  ...
  scenarios), `action-05` (the hostile-ambient scenario — fixture 4's
  planted ambient, verbatim — paired byte-for-byte with its clean run),
  `action-06` (the drift row's paired files).
...
- **No port bypass.** No store mutation outside the doors, no direct ref
  ...
  another name. One carved exception, declared: **seeding recorded state in
  the fixture's own repository before the run** is fixture setup, not a
  bypass — the pre-created `.lock` files that seat `git-07`'s ambiguity
  window (the CLI harness's own mechanism), the seeded line heads and
  channel refs the binding then reads. What is seeded is recorded evidence
  the doors consume; nothing is seeded to skip a check the doors would
  have run. Enforcement: the manifest's wrapper column, each seeded cell's
  row naming its seed, plus review.
...
- **The generator and the suite drift into two runners** — the suite
  ...
  refusal, and `git-15`'s pairing would flake if the two drives diverged in
  their projection.
- **The projection hides a real nondeterminism.** A second random value
  ...
  the pairing — `git-15`'s cell, executable against both assemblies.
- **The matrix quietly omits an interruption window** and a recorded-evidence
  ...
  ([§5](#5-the-change-protocol)): every recorded window is claimed by the
  earned rows or the scoped refusals — `earned ∪ refused == taxonomy`,
  `earned ∩ refused == ∅` — and the suite enforces both.
...
2. **Matrix breadth versus suite runtime.** The repository's suite already
   ...
   **40 live cells** — the §3.3 table's thirty-five live cells (thirty-six
   earned rows, one census-only) leaves headroom for the change protocol's
   additions. If a future slice busts the budget, the
   ...
   posture minimum (`x-01` through `x-03`); and if the budget is still
   bust after the third, the slice files the budget defect and the cap is
   renegotiated in review — never silently exceeded. The full-matrix dream
   — every window on every assembly — is refused by this rule rather than
   by silence.
