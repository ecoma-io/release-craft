...
| `git-13`    | —                                                     | A-git (nominally)     | CLI                                        | typed rows + renderings                                                                                  | Census-only, declared: the exit table's seven unreachable-today rows (`satisfied-externally`, `resolved`, `abandoned`, `blocked`, `failed`, `stale`, `escalate` — exactly phase 12 §6.2's class) are unreachable through every legal transport (R1: the declarations-less process cannot produce them, and the import law of [§8](#8-what-the-fixture-never-does) bars the fixture from `src/cli/`, where a renderer-level pin would live), so this row produces no test. Its enforcement is named in its manifest row: the exit-table cross-pin in the CLI's own suite plus this contract's recorded copy of the table ([§4.2](#42-the-classes)). The row exists so the fixture's census of the table is total, and it is exempt from the produced-by-a-test rule ([§5](#5-the-change-protocol)). |
