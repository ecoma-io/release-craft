...
- **Hook is deferred, with trigger.** The taxonomy classifies hooks as an
  adapter ("user code must never enter the pure kernel"), the matrix's
  canonical vocabulary does not define the term, and the hooks runtime stays
  out of the near phases per [#13](https://github.com/ecoma-io/release-craft/issues/13).
  ...
  plan-mutating hooks refused by construction — becomes testable. The direction
  is locked now (execution-side step, never a planning participant); only the
  seam's shape waits.
...
5. **Plan identity is its fingerprint.** Two plans with the same target version
   ...
   objects, and one supersedes the other through a recorded relation rather
   than being edited in place. _Stress: PL-08, P-05, E-04, E-11. First
...
- artifact sets, generations, and evidence freshness rules (AR-01, PR-02,
  PR-03);
- hooks, once the execution phase defines the step seam (deferred above).
...
[sixteen open forks](release-scenarios.md#open-forks-this-matrix-deliberately-retains)
